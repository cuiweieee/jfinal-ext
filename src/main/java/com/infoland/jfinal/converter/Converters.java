...
public class Converters {
		...
		public Integer convert(String s) {
			...
			if ((s.startsWith("N")) || (s.startsWith("n"))) {
				if (s.startsWith("number:")) {
					return Integer.valueOf(Integer.parseInt(s.replace("number:", "").trim()));
				}
				return Integer.valueOf(-Integer.parseInt(s.substring(1)));
			}
	...
	public static class LongConverter implements IConverter<Long> {
		...
		public Long convert(String s) {
			s = s.trim();
			if ((s.startsWith("N")) || (s.startsWith("n")))
				return Long.valueOf(-Long.parseLong(s.substring(1)));
	...
	public static class DateConverter implements IConverter<java.util.Date> {
		...
		// java.sql.Time,java.sql.Timestamp 全部直接继承自 java.util.Data, 所以 getDate可以返回这三类数据
		@Override
